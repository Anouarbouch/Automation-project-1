...
describe('This is first test suite', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        ...
        cy.get('#firstName').type('sakn')
        cy.get('#lastName').type('Ddsds')
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="password"]').type('Passll')
        cy.get('[name="confirm"]').type('Passll')
        //25.09.2024
    ...
    */
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        cy.get('#username').type('kudo')
        cy.get('[data-testid="phoneNumberTestId"]').type('55995850')
        cy.get('#firstName').type('kohe')
        cy.get('#lastName').type('koe')
        cy.get('input[name="password"]').type('Passll')
        cy.get('.submit_button').should('be.disabled')
        cy.get('#success_message').should('not.be.visible')
